...
const MAIL = "nikitaromanov60@gmail.com";
let emojiList;
...
function screen_1() {
  ...
  part1.addEventListener("click", () => {
    ...
    content.innerHTML = `<span><strong>Purpose of the application</strong></span>
    <ul>
      <li>Display Emojis</li>
      <li>Display ServiceWorker log's</li>
    </ul>
    <span><strong>Functional</strong></span>
    <ul>
      <li>Installing the application via the A2HS interface</li>
      <li>All information is cached</li>
      <li>Implemented "smart" loading</li>
      <li>Receiving data via REST API</li>
      <li>Implemented ServiceWorker log</li>
      <li>The certificate is implemented on the hosting side</li>
    </ul>`;
    if (window.matchMedia("(display-mode: fullscreen)").matches) {
      document.querySelector(".install_button").classList.add("hidden");
    } else {
      content.innerHTML += `<button class="install_button" onclick='install()'>Download</button>`;
    }
  });
  part1.click();
}
...
function screen_3() {
  ...
  part3.addEventListener("click", () => {
    ...
    content.innerHTML = `<img src='icon.png'><span>Nick Romanov</span><span>${MAIL}</span>`;
  });
}
